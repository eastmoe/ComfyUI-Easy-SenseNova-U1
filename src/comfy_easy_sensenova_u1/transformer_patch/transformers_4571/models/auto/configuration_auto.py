...
class _LazyConfigMapping(OrderedDict[str, type[PretrainedConfig]]):
    ...
    def __getitem__(self, key: str) -> type[PretrainedConfig]:
        ...
        if module_name not in self._modules:
            self._modules[module_name] = importlib.import_module(f".{module_name}", "transformers_4571.models")
        if hasattr(self._modules[module_name], value):
            return getattr(self._modules[module_name], value)

        # Some of the mappings have entries model_type -> config of another model type. In that case we try to grab the
        # object at the top level.
        transformers_module = importlib.import_module("transformers_4571")
...
class AutoConfig:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, os.PathLike[str]], **kwargs):
        ...
        ```python
        >>> from transformers_4571 import AutoConfig
