...
def normalize_image(image: Image.Image) -> torch.Tensor:
    """Convert an RGB PIL image to a normalized CHW float tensor."""
    array = np.array(image, dtype=np.uint8, copy=True)
    tensor = torch.from_numpy(array).permute(2, 0, 1).to(dtype=torch.float32).div_(255.0)
...
def load_image_native(
    ...
    pixel_values, grid_hw = preprocess_pixel_values(
        normalize_image(new_image), patch_size=patch_size
    )
